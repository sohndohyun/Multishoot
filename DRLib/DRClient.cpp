...
int dr_client::init(const char* ip, int port) {
    if (is_initialized_)
        return 1;
...
void dr_client::send_data(char* data, int size) {
    if (!running_ || !connected_ || data == nullptr || size < 0 ||
        size > dr_packet::max_data_size())
        return;
...
unsigned int _stdcall dr_client::io_thread_main(void* client_class) {
    auto* client = static_cast<dr_client*>(client_class);
    while (true) {
        DWORD bytes_trans = 0;
...
unsigned int _stdcall dr_client::send_thread(void* client_class) {
    auto* client = static_cast<dr_client*>(client_class);
...
unsigned int _stdcall dr_client::callback_thread(void* client_class) {
    auto* client = static_cast<dr_client*>(client_class);
    while (true) {
        client->on_update();
        process_info info;
...
bool dr_client::receive_process(char* data, int size) {
...
bool dr_client::post_receive(io_info* io) {
