...
int server::init(int port, int thread_count) {
    if (is_initialized_)
        return 1;
...
void server::send_data(SOCKET socket, char* data, int size) {
    if (!running_ || data == nullptr || size < 0 || size > packet::max_data_size())
        return;
...
unsigned int _stdcall server::accept_thread_main(void* server_class) {
    auto* server = static_cast<dr::server*>(server_class);
...
unsigned int _stdcall server::io_thread_main(void* server_class) {
    auto* server = static_cast<dr::server*>(server_class);
    while (true) {
        DWORD bytes_trans = 0;
...
unsigned int _stdcall server::send_thread(void* server_class) {
    auto* server = static_cast<dr::server*>(server_class);
...
unsigned int _stdcall server::callback_thread(void* server_class) {
    auto* server = static_cast<dr::server*>(server_class);
...
bool server::receive_process(client_context* client, char* data, int size) {
...
bool server::post_receive(client_context* client, io_info* io) {
    if (client->closing)
        return false;
...
void server::close_client(client_context* client, bool notify) {
...
void server::add_reference(client_context* client) {
    client->references.fetch_add(1);
}

void server::release_client(client_context* client) {
